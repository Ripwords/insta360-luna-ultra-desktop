...
const DEVICE_CONFIG = JSON.parse(
  readFileSync(new URL("./luna-ultra.json", import.meta.url), "utf-8"),
);
const STORAGE_PATHS = DEVICE_CONFIG.storages.map((s) => s.path);
const CAMERA_PATH =
  STORAGE_PATHS.find((p) => DEVICE_CONFIG.storages[STORAGE_PATHS.indexOf(p)].default) ||
  STORAGE_PATHS[0] ||
  "/";
...
const httpPort = Number(
  argValue("--http-port") || process.env.LUNA_MOCK_HTTP_PORT || DEFAULT_MOCK.httpPort,
);
const tcpPort = Number(
  argValue("--tcp-port") || process.env.LUNA_MOCK_TCP_PORT || DEFAULT_MOCK.tcpPort,
);
const rateBps =
  Number(argValue("--rate-mbps") || process.env.LUNA_MOCK_RATE_MBPS || DEFAULT_MOCK.rateMbps) *
  1024 *
  1024;
...
function stripCameraDir(relative) {
  ...
  return dir && !hasExplicitCameraDir(dir)
    ? normalized.slice(dir.length).replace(/^\/+/, "")
    : normalized;
}
...
function requestRelativePath(pathname, matchedStorage) {
  ...
  return matchedStorage === "/" ? decodedPath.slice(1) : decodedPath.slice(matchedStorage.length);
}
...
`;
  ...
  for (const filePath of files) {
    ...
    const baseDir =
      cameraDir && filePath.startsWith(path.join(rootDir, cameraDir) + path.sep)
    ...
    const cameraPath = decodeURIComponent(
      new URL(href, `http://luna.mock${normalizedPath}`).pathname,
    );
...
const httpServer = createHttpServer(async (request, response) => {
  try {
    const url = new URL(
      request.url || "/",
      `http://${request.headers.host || `${host}:${httpPort}`}`,
    );
    const matchedStorage = storagePathForRequest(url.pathname);
    if (!matchedStorage) {
    ...
    if (isIndexRequest(url.pathname, matchedStorage)) {
      ...
      // 非默认存储路径返回空目录，避免素材重复
      response.end(
        matchedStorage === CAMERA_PATH || matchedStorage === "/"
          ? await indexHtml(url.pathname)
...
const tcpServer = createTcpServer((socket) => {
  ...
  socket.on("data", (chunk) => {
    ...
    if (
      received.length <= EXPECTED_AUTH.length &&
      received.equals(EXPECTED_AUTH.subarray(0, received.length))
    ) {
...
async function main() {
  await stat(rootDir);
  await Promise.all([listen(httpServer, httpPort, host), listen(tcpServer, tcpPort, host)]);
